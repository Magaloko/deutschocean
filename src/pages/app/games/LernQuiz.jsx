  ...
  if (phase === 'levelSelect') {
    return (
      <div className={styles.page}>
        <button className={styles.backBtn} onClick={() => navigate('/app')}>← Zurück</button>
        <div className={styles.header} style={{ '--color': fachColor }}>
          <span className={styles.headerEmoji}>{fachEmoji}</span>
    ...
    return (
      <div className={styles.page}>
        <div className={styles.resultCard} style={{ '--color': fachColor }}>
          <div className={styles.resultEmoji}>{fachEmoji}</div>
          ...
          <div className={styles.resultBtns}>
            <button className={styles.btnPrimary} style={{ background: fachColor }} onClick={handleFinish}>
              ✓ Fertig & XP kassieren
            </button>
            <button className={styles.btnSecondary} onClick={() => startLevel(activeLevel)}>
              🔄 Nochmal spielen
            </button>
          </div>
        </div>
  ...
  return (
    <div className={styles.page}>
      <div className={styles.topBar}>
        <button className={styles.backBtn} onClick={() => setPhase('levelSelect')}>← Level</button>
        ...
        <span className={styles.scoreChip} style={{ background: `${fachColor}20`, color: fachColor }}>
          ✓ {score}
        </span>
      </div>

      <div className={styles.levelPill} style={{ background: `${fachColor}18`, color: fachColor }}>
        ...
        {currentQ.shuffledOptions.map((opt, i) => {
          ...
          return (
            <button key={i} className={cls} onClick={() => handleSelect(i)} disabled={answered}>
              ...
              {answered && opt.correct && <span className={styles.tick}>✓</span>}
              {answered && !opt.correct && i === selected && <span className={styles.cross}>✗</span>}
            </button>
          )
        })}
      </div>
      ...
      {answered && (
        <div className={styles.feedback}>
          {currentQ.shuffledOptions[selected]?.correct
            ? <span className={styles.feedbackCorrect}>🎉 Richtig! Weiter so!</span>
            : <span className={styles.feedbackWrong}>❌ Nicht ganz — die grüne Antwort war richtig!</span>}
          <button className={styles.nextBtn} style={{ background: fachColor }} onClick={handleNext}>
            {idx + 1 >= total ? '📊 Ergebnis sehen' : 'Weiter →'}
          </button>
        </div>
      )}
    </div>
