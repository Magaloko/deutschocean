// AUTO-PORTIERT von fachData.js → TypeScript

export interface FachFrage { q: string; options: string[]; a: number; erklaerung?: string }
export interface FachLevel { label: string; emoji: string; desc: string }
export interface Fach {
  id: string; label: string; emoji: string; color: string
  levels: Record<number, FachLevel>
  fragen: Record<number, FachFrage[]>
  missionPrefix: string; route: string; bookSource?: string
}
