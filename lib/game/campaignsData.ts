// AUTO-PORTIERT von campaignsData.js → TypeScript

export type StepType = 'mission' | 'choice'

export interface CampaignStep {
  id: string; type: StepType; label: string; icon: string
  storyIntro?: string; storyOutro?: string
  missionId?: string; route?: string
  dilemma?: string; options?: Array<{ id: string; label: string; icon: string; reaction: string }>
}

export interface Campaign {
  id: string; title: string; subtitle: string; icon: string; color: string
  gradient: string; intro: string; modules?: string[]
  reward: { xp: number; badgeId?: string; title: string; message: string }
  steps: CampaignStep[]
}
...
function isStepComplete(step: CampaignStep, profile: ProfileLike | null, campaignId: string) {
  if (step.type === 'choice') {
...
interface ProfileLike {
  ...
  campaignProgress?: Record<string, { completedAt?: string; choices?: Record<string, string> }>
}

export function getCampaignStatus(campaign: Campaign, profile: ProfileLike | null) {
...
export function getStepChoice(campaign: Campaign, stepId: string, profile: ProfileLike | null) {
...
export function isCampaignForModule(campaign: Campaign, schoolModule: string) {
  return campaign.modules?.includes(schoolModule) ?? true
}
